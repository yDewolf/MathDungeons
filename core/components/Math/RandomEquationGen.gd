...
func random_number() -> float:
	return _random_number(integer_range, decimal_range, inclusive)

static func _random_number(max_digits: int, decimal_digit_range: float, inclusive_range: bool) -> float:
	var offset: int = 0
...
static func _generate_expression_string(min_op: int, max_op: int, allowed_ops: Array[MathOperation.OperationTypes], max_digits: int, decimal_digit_range: float, inclusive_range: bool) -> String:
	...
	for idx in op_amount:
		...
		if idx == 0:
			var first_n = _random_number(max_digits, decimal_digit_range, inclusive_range)
			expression_string += str(first_n) + " "
		
		var second_n = _random_number(max_digits, decimal_digit_range, inclusive_range)
